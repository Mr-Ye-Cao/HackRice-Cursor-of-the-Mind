...
def newton_dou(fx, v):
    m = tf.Variable([0,0,0.1 * np.pi,0.02])
    aim, _ = fx(m)

    optimizer = tf.train.AdadeltaOptimizer(1., 0.95, 1e-6)
    train_op = optimizer.minimize(aim)

    accum = optimizer.get_slot(m, "accum")  # accumulator of the square gradients
    accum_update = optimizer.get_slot(m, "accum_update")  # accumulator of the square updates
    ...
    for i in range(100):
        sess.run(train_op)
        print(sess.run([m, accum, accum_update]))